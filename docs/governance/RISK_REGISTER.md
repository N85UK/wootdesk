...
| RISK-012 | A push provider routes a notification to the wrong profile, account, environment, or device | 2 | 5 | 10 | Use authenticated per-profile enrolment, opaque payload identifiers, webhook signature and replay checks, token rotation and deletion, and cross-profile physical-device acceptance | Open, provider not implemented |
