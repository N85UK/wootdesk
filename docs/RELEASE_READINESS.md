...
| GO-012 | Remote new-message notifications are private, profile-safe, and reliable | Source complete. Per-agent routing implemented per `DEC-008`: an assigned conversation reaches only the assignee, an unassigned one reaches every agent on the account. 22 gateway tests and 196 Swift tests pass, and the payload carries no message content. Not exercised against a real APNs delivery; gateway deployment and physical acceptance remain | In progress; source complete, delivery acceptance blocked |
