...
Status: Both platforms rejected for information and being prepared for resubmission. Cause found and fixed on 10 September 2026; screen recordings outstanding
...
| GO-013 | App Review outcome | **Both platforms rejected under Guideline 2.1, Information Needed**: macOS build 100 on 5 September 2026, and iOS build 98 in a message dated 6 September with the same text. Apple asks for a screen recording on a physical device running the latest operating system, starting at launch, plus written answers on purpose, setup, external services, regional differences and regulated material. The written answers are in the App Review Notes on both platforms. **Build 112 is attached to macOS and build 111 to iOS**; 111 drops the unimplemented AI settings section that 98 still contained. Outstanding: the recordings on the physical iPhone (iOS 27.0, build 24A435) and on the Mac (macOS 27.0, build 26A428), then the reply and resubmission on both platforms | Recordings outstanding |
| GO-006 | App Store metadata and screenshots are approved | **Both platforms submitted on 4 September 2026** and both rejected under Guideline 2.1, Information Needed: macOS on 5 September, and iOS in a message dated 6 September with the same request. **Cause found on 10 September**: the demo credentials opened an empty Chatwoot account, see GO-008. iOS now carries build 111 and macOS build 112. Resubmission is prepared apart from the screen recordings, see GO-013. Owner approval of this content belongs to GO-010 | Rejected for information; resubmission prepared, recordings outstanding |
| GO-007 | Privacy and export-compliance answers are approved | **Export compliance:** the current source declares `ITSAppUsesNonExemptEncryption = false` in `Info-Release.plist`, verified 10 September 2026. **Privacy: published and verified on 10 September.** The live answer had still been "Data Not Collected" while the privacy manifest declares collection. It now declares User ID and Device ID, both for App Functionality, linked to the user and not used for tracking, confirmed from the page text after publishing. While it was being entered, Device ID was briefly published as used for tracking, because browser clicks were placed from screenshot coordinates in a different frame from the page's real viewport. It was corrected within minutes by clicking by element reference and verifying each step before publishing | Published and verified |
| GO-008 | Dedicated App Review server and account are ready | **Ready, and the cause of both rejections fixed on 10 September 2026.** `review.n85.app` returned HTTP 200. The credentials in App Store Connect belonged to a second Chatwoot user, created with its own empty account on 4 September, five minutes before submission, rather than to the seeded `Demo Agent`, so the reviewer saw no conversations. That user is now an administrator of the seeded account `WootDesk Demo Support` and a member of its inbox, and no longer belongs to the empty account, so the app auto-selects the seeded account. Verified with the token App Store Connect holds: one account, 2 open conversations, 3 in total. The credentials in App Store Connect were not changed | Ready; verified with the reviewer's credentials |
...
| GO-010 | Product, security, and release owners record Go | **Release owner approved on 10 September 2026**, recorded in the table below. Product, technical and security decisions are still pending. Both platforms were first submitted on 4 September before any decision was recorded, so that approval followed the first submission rather than preceding it | Release owner recorded; others pending |
...
| Release owner | Paul McCann | Go | 10 September 2026 | Resubmission of iOS build 111 and macOS build 112 once the screen recordings are attached |
