...
| GO-012 | Remote new-message notifications are private, profile-safe, and reliable | Client and gateway source, deterministic enrolment, rotation, deletion, filtering, and account-isolation tests pass, and the payload carries no message content. **N85-15 AC2 is unmet in source**: recipients are selected by Chatwoot account, so every enrolled device on a shared account receives the alert regardless of assignee. See `DEC-008`. Gateway deployment and physical delivery also remain | In progress; **source incomplete**, AC2 unimplemented |
